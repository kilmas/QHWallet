...
export default class EOSWallet extends Wallet {
  ...
  getBalance = async () => {
    const { eosjs } = Scatter
    if (this.name && this.hasCreated && eosjs) {
      try {
        const { rows } = await eosjs.getTableRows(true, "eosio.token", this.name, "accounts", "primary", 0, 5000, 5000);
        if (Array.isArray(rows)) {
          rows.forEach(item => {
            const [balance, token] = _.get(item, 'balance.quantity', '').split(' ')
            if (token === 'EOS' && this.coins[0]) {
