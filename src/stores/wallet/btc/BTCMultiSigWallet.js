...
import { installID } from "../../../../utils/device";
...
class BTCMultiSigWallet extends MultiSigWallet {
  ...
  get HDWallet() {
    return this.accountStore.defaultHDAccount.BTCWallet;
  }
  ...
  static create = async ({ walletName, nick, total, required, extendedPublicKey }) => {
    ...
    this.accountStore.defaultMultiSigAccount.wallets.splice(0, 0, wallet);
  };
  ...
  static recover = async ({ walletID }) => {
    ...
    this.accountStore.defaultMultiSigAccount.wallets.splice(0, 0, wallet);
    try {
      await this.accountStore.defaultMultiSigAccount.update();
    } catch (error) {}
  };
  constructor(obj) {
    const { type, required, total, members } = obj;
    super(obj);
    this.accountStore = obj.accountStore
    this.type = 0;
  ...
  sendRawTransaction = async (to, amount, feePerByte, pwd, note, retry = true) => {
    ...
    try {
      ...
      this.accountStore.defaultMultiSigAccount.pendingTxs.splice(0, 0, multisigTx);
  ...
  isVaildPassword = pwd => this.accountStore.defaultHDAccount.BTCWallet.isVaildPassword(pwd);
  /**
   *
