...
import Networks, { getAllNetworks } from '../../../utils/networks'
...
class NetworkList extends React.Component {
  ...
  renderOtherNetworks = () => {
    ...
    return this.getOtherNetworks().map((network, i) => {
      ...
      const selected = provider.type === network ? <Icon name="check" size={20} color={colors.fontSecondary} /> : null
  ...
  renderRpcNetworks = () => {
    ...
    return frequentRpcList.map(({ nickname, rpcUrl }, i) => {
      ...
      const selected = provider.rpcTarget === rpcUrl && provider.type === 'rpc' ? <Icon name="check" size={20} color={colors.fontSecondary} /> : null
  ...
  renderMainnet() {
    ...
    const isMainnet = provider.type === 'mainnet' ? <Icon name="check" size={15} color={colors.fontSecondary} /> : null
    ...
    return (
      <View style={styles.mainnetHeader}>
        <TouchableOpacity
          ...
          <View>
            <Text style={styles.mainnetStatus}>{networkStatus.infura.mainnet ? strings('networks.status_ok') : strings('networks.status_not_ok')}</Text>
          </View>
        </TouchableOpacity>
      </View>
    )
...
export default inject(({ store: { engine: { backgroundState } } }) => ({
  provider: backgroundState.NetworkController.provider,
  frequentRpcList: backgroundState.PreferencesController.frequentRpcList,
  networkStatus: backgroundState.NetworkStatusController.networkStatus,
}))(observer(NetworkList))
