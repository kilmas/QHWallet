...
import NetworkList, { getNetworkTypeById, findBlockExplorerForRpc, getBlockExplorerName } from '../../../../utils/networks'
  ...
  viewOnEtherscan = () => {
    ...
    try {
      if (type === 'rpc') {
        ...
        goBrowser(this.props.navigation, url)
  ...
  render() {
    ...
    return (
      <View style={styles.detailRowWrapper}>
        ...
        <View style={[styles.section, styles.flexRow, !!nonce && styles.sectionBorderBottom]}>
          <View style={[baseStyles.flexGrow, styles.flexRow]}>
            <View style={baseStyles.flexRow}>
              ...
              <EthereumAddress type="short" style={styles.detailRowText} address={this.props.transactionDetails.renderFrom} />
            </View>
            <View style={styles.flexEnd}>
              <Text style={styles.detailRowTitle}>{strings('transactions.to')}</Text>
              <EthereumAddress type="short" style={styles.detailRowText} address={this.props.transactionDetails.renderTo} />
            </View>
          </View>
        </View>
        {!!nonce && (
        ...
        {this.props.transactionDetails.transactionHash && transactionObject.status !== 'cancelled' && rpcBlockExplorer !== NO_RPC_BLOCK_EXPLORER && (
          <TouchableOpacity onPress={this.viewOnEtherscan} style={styles.touchableViewOnEtherscan}>
            <Text style={styles.viewOnEtherscan}>
              {(rpcBlockExplorer && `${strings('transactions.view_on')} ${getBlockExplorerName(rpcBlockExplorer)}`) ||
