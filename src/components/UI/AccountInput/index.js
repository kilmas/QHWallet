...
import { renderShortAddress, isENS } from '../../../utils/address'
    ...
    if (typeof value !== 'undefined' && value.toString().length > 0) {
      ...
      Object.keys(allAddresses).forEach(address => {
        if (
          ...
          (allAddresses[address].name && allAddresses[address].name.toLowerCase().indexOf(value.toLowerCase()) !== -1)
        ) {
        ...
        <View style={styles.accountContainer}>
          ...
          <View style={styles.inputContainer}>
            <View style={styles.toContainer}>
              ...
              />
              <View style={styles.ensView}>{!!ensRecipient && <Text style={styles.ensAddress}>{renderShortAddress(address)}</Text>}</View>
            </View>
          </View>
          <Icon onPress={this.onFocus} name={'arrow-drop-down'} size={24} style={styles.arrow} testID={'account-drop-down'} />
        </View>
...
}))(observer(withNavigation(AccountInput)))