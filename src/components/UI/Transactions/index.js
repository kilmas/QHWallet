...
import { ScrollView, ActivityIndicator, RefreshControl, StyleSheet, Text, View, FlatList, Dimensions, InteractionManager } from 'react-native'
...
class Transactions extends React.Component {
  ...
  render() {
    ...
    const transactions = submittedTransactions && submittedTransactions.length ? submittedTransactions.concat(confirmedTransactions) : this.props.transactions
    ...
    return (
      <View style={styles.wrapper}>
        ...
        <TransactionActionModal
          ...
          feeText={`${renderFromWei(Math.floor(this.existingGasPriceDecimal * CANCEL_RATE))} ${strings('unit.eth')}`}
        ...
        <TransactionActionModal
          ...
          confirmText={strings('transaction.lets_try')}
          cancelText={strings('transaction.nevermind')}
          feeText={`${renderFromWei(Math.floor(this.existingGasPriceDecimal * SPEED_UP_RATE))} ${strings('unit.eth')}`}
...
export default inject(({ store: state }) => ({
  ...
  showAlert: config => state.showAlert(config),
