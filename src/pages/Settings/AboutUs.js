...
class AboutUs extends React.Component {
  updateAlert = () => {
    Alert.alert(strings('Help'), strings('Update installed, please restart APP!'), [
      {
        text: 'Ok',
        onPress: () => {
          console.log('点了OK')
    ...
    return (
      <Container>
        ...
        <KeyboardAwareScrollView contentContainerStyle={themeStyles.pt26}>
          ...
          <Text style={styles.walletText}>Qingah Wallet</Text>
          <Text style={styles.descText}>{strings('settings.aboutDesc')}</Text>
          <Button type={newVersion ? 'warning' : 'primary'} onPress={() => {}}>
            {newVersion ? strings('settings.newVersion') : strings('settings.noVersion')}
          </Button>
