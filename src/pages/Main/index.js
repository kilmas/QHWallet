...
import { BNToHex, hexToBN, fromWei, renderFromTokenMinimalUnit } from '../../../utils/number'
...
import { CONNEXT_DEPOSIT, getMethodData, TOKEN_METHOD_TRANSFER, decodeTransferData, APPROVE_FUNCTION_SIGNATURE } from '../../../util/transactions'
...
class Main extends React.Component {
            ...
            },
            {
              ...
              onPress: () => {
                ...
                InteractionManager.runAfterInteractions(() => {
                  setTimeout(() => {
                    this.props.navigation.navigate('BrowserView', {
                      newTabUrl: 'https://blog.makerdao.com/what-to-expect-with-the-launch-of-multi-collateral-dai/',
                    })
                    const tsToRemind = Date.now() + AppConstants.SAI_MIGRATION_DAYS_TO_REMIND
                    AsyncStorage.setItem('@MetaMask:nextMakerReminder', tsToRemind.toString())
                  }, 300)
  ...
  initializePaymentChannels = () => {
    ...
    PaymentChannelsClient.hub.on('payment::request', async request => {
      ...
      if (isNaN(request.amount)) {
        Alert.alert(strings('payment_channel_request.title_error'), strings('payment_channel_request.amount_error_message'))
        return
      }
      ...
      if (isInvalidAddress || (!isAddress && !isENS(request.to))) {
        Alert.alert(strings('payment_channel_request.title_error'), strings('payment_channel_request.address_error_message'))
        return
      }
    ...
    PaymentChannelsClient.hub.on('payment::error', error => {
      if (error === 'INVALID_ENS_NAME') {
        ...
        Alert.alert(strings('payment_channel_request.error'), strings('payment_channel_request.balance_error_message'))
      }
  ...
  renderSigningModal = () => {
    const { signMessage, signMessageParams, signType, currentPageTitle, currentPageUrl, showExpandedMessage } = this.state
    return (
      <Modal
  ...
  renderWalletConnectReturnModal = () => <WalletConnectReturnToBrowserModal modalVisible={this.state.walletConnectReturnModalVisible} />

  renderPaymentChannelRequestApproval = () => {
    const { paymentChannelRequest, paymentChannelRequestInfo, paymentChannelRequestLoading, paymentChannelRequestCompleted } = this.state
    ...
    return (
      <React.Fragment>
        <View style={styles.flex}>
          {!forceReload ? (
            <MainNavigator navigation={this.props.navigation} screenProps={{ isPaymentChannelTransaction, isPaymentRequest }} />
          ) : (
            this.renderLoader()
          )}
          <FadeOutOverlay />
