...
import { InteractionManager, StyleSheet, SafeAreaView, View, Alert, Text, ScrollView, TouchableOpacity, ActivityIndicator } from 'react-native'
...
import { fetchBasicGasEstimates, convertApiValueToGWEI } from '../../../utils/custom-gas'
import collectiblesTransferInformation from '../../../utils/collectibles-transfer'
import { getTicker, decodeTransferData } from '../../../utils/transactions'
...
import TransactionReviewFeeCard from '../../../components/UI/TransactionReview/TransactionReviewFeeCard'
...
class Confirm extends React.Component {
  ...
  componentDidUpdate = (prevProps, prevState) => {
    ...
    const recipientIsDefined = !!transactionTo // (transactionTo !== undefined)
  ...
  parseTransactionData = () => {
    ...
    let fromAccountBalance, transactionValue, transactionValueFiat, transactionTo, transactionTotalAmount, transactionTotalAmountFiat
    ...
    if (isPaymentChannelTransaction) {
      ...
      const collectibleTransferInformation =
        selectedAsset.address.toLowerCase() in collectiblesTransferInformation && collectiblesTransferInformation[selectedAsset.address.toLowerCase()]
      ...
      fromAccountBalance = `${renderFromTokenMinimalUnit(contractBalances[address] ? contractBalances[address] : '0', decimals)} ${symbol}`
      ...
      transactionValueFiat = balanceToFiat(transferValue, conversionRate, exchangeRate, currentCurrency) || `0 ${currentCurrency}`
      ...
      transactionTotalAmountFiat = renderFiatAddition(transactionValueFiatNumber, transactionFeeFiatNumber, currentCurrency)
    }
  ...
  toggleCustomGasModal = () => {
    this.setState(state=> ({ customGasModalVisible: !state.customGasModalVisible }))
  }

  toggleHexDataModal = () => {
    this.setState(state => ({ hexDataModalVisible: !state.hexDataModalVisible }))
  }
      ...
      <Modal
        ...
        <View style={styles.hexDataWrapper}>
          <TouchableOpacity style={styles.hexDataClose} onPress={this.toggleHexDataModal}>
            <Icon name={'ios-close'} size={28} color={colors.black} />
          </TouchableOpacity>
  ...
  onNext = async () => {
    ...
    try {
      ...
      const { result, transactionMeta } = await TransactionController.addTransaction(transaction, TransactionTypes.MMM)
  ...
  render = () => {
    ...
    console.log(this.props.transactionState, fromSelectedAddress, transactionTo)
    return (
      <SafeAreaView style={styles.wrapper} testID={'txn-confirm-screen'}>
          ...
          {!selectedAsset.tokenId ? (
            ...
            <View style={styles.amountWrapper}>
              ...
              <View style={styles.collectibleImageWrapper}>
                <CollectibleImage iconStyle={styles.collectibleImage} containerStyle={styles.collectibleImage} collectible={selectedAsset} />
              </View>
              <View>
        ...
        <View style={styles.buttonNextWrapper}>
          <Button style={styles.buttonNext} type="primary" onPress={isPaymentChannelTransaction ? this.onPaymentChannelSend : this.onNext} disabled={!gasEstimationReady || Boolean(errorMessage)} loading={transactionConfirmed}>
            Send
          </Button>
        </View>
...
}))(observer(withNavigation(Confirm)))
