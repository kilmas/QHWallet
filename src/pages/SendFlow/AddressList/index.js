...
import { decodeTransferData, TRANSFER_FUNCTION_SIGNATURE, TRANSFER_FROM_FUNCTION_SIGNATURE } from '../../../utils/transactions'
...
class AddressList extends React.Component {
  ...
  componentDidMount = () => {
    ...
    const networkAddressBookList = Object.keys(this.networkAddressBook).map(address => this.networkAddressBook[address])
  ...
  getRecentAddresses = inputSearch => {
    ...
    const parsedRecents = []
    if (!inputSearch) {
      const networkTransactions = transactions.filter(tx => tx.networkID === network).sort((a, b) => b.time - a.time)
      networkTransactions.forEach(async ({ transaction: { to, data } }) => {
        ...
        if (!recents.includes(checksummedTo) && !Object.keys(identities).includes(checksummedTo)) {
          ...
          if (this.networkAddressBook[checksummedTo]) {
            ...
            parsedRecents.push(
              <AddressElement key={checksummedTo} address={checksummedTo} onAccountPress={onAccountPress} onAccountLongPress={onAccountLongPress} />
            )
          }
        }
      })
  ...
  renderMyAccounts = () => {
    ...
    return !myAccountsOpened ? (
      <TouchableOpacity style={styles.myAccountsTouchable} onPress={this.openMyAccounts} testID={'my-accounts-button'}>
    ...
    return (
      <View style={styles.root}>
        <ScrollView style={styles.myAccountsWrapper}>
          ...
          {contactElements.length ? <FlatList data={contactElements} keyExtractor={this.elementKeyExtractor} renderItem={this.renderElement} /> : null}
        </ScrollView>
      </View>
    )
  }
