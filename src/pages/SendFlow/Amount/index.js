...
import { StyleSheet, Text, SafeAreaView, View, TouchableOpacity, TextInput, KeyboardAvoidingView, FlatList, Image, InteractionManager } from 'react-native'
...
import { getTicker, generateTransferData, getEther } from '../../../utils/transactions'
import { util } from '@metamask/controllers'
import { Button, Icon } from '@ant-design/react-native'
import FadeIn from 'react-native-fade-in-image'
import ErrorMessage from '../ErrorMessage'
import { fetchBasicGasEstimates, apiEstimateModifiedToWEI } from '../../../utils/custom-gas'
import collectiblesTransferInformation from '../../../utils/collectibles-transfer'
...
class Amount extends React.Component {
  ...
  componentDidMount = async () => {
    ...
    if (paymentChannelTransaction || paymentRequest || !selectedAsset.tokenId) {
      this.handleSelectedAssetBalance(selectedAsset, paymentChannelTransaction ? selectedAsset.assetBalance : null)
    }
  ...
  onNext = async () => {
    ...
    if (transaction.value !== undefined && !!transaction.to) {
  ...
  getCollectibleTranferTransactionProperties() {
    ...
    if (!collectibleTransferInformation || (collectibleTransferInformation.tradable && collectibleTransferInformation.method === 'transferFrom')) {
  ...
  useMax = () => {
    ...
    if (paymentChannelTransaction) {
      ...
      if (internalPrimaryCurrencyIsCrypto || !exchangeRate) {
        ...
        input = `${balanceToFiatNumber(fromTokenMinimalUnit(contractBalances[selectedAsset.address], selectedAsset.decimals), conversionRate, exchangeRate)}`
      }
    }
  ...
  onInputChange = (inputValue, selectedAsset) => {
    ...
    if (selectedAsset.isETH) {
      ...
      if (internalPrimaryCurrencyIsCrypto) {
        ...
        renderableInputValueConversion = `${weiToFiat(toWei(processedInputValue), conversionRate, currentCurrency)}`
      } else {
      ...
      if (internalPrimaryCurrencyIsCrypto || !hasExchangeRate) {
        ...
        renderableInputValueConversion = `${balanceToFiat(processedInputValue, conversionRate, exchangeRate, currentCurrency)}`
      } else {
        inputValueConversion = `${renderFromTokenMinimalUnit(
          fiatNumberToTokenMinimalUnit(processedInputValue, conversionRate, exchangeRate, selectedAsset.decimals),
  ...
  renderToken = (token, index) => {
    const { accounts, selectedAddress, conversionRate, currentCurrency, contractBalances, contractExchangeRates } = this.props
  ...
  processCollectibles = () => {
    ...
    this.props.collectibles
      ...
      .forEach(collectible => {
        ...
        const isTradable = !collectiblesTransferInformation[address] || collectiblesTransferInformation[address].tradable
        if (!isTradable) return
        const collectibleContract = collectibleContracts.find(contract => contract.address.toLowerCase() === address)
    ...
    return (
      <Modal
        ...
        <SafeAreaView style={styles.assetsModalWrapper}>
          ...
          <FlatList data={[...this.tokens, ...this.collectibles]} keyExtractor={this.assetKeyExtractor} renderItem={this.renderAsset} />
        </SafeAreaView>
      </Modal>
    )
  }
  ...
  renderTokenInput = () => {
    const { inputValue, renderableInputValueConversion, amountError, hasExchangeRate, internalPrimaryCurrencyIsCrypto, currentBalance } = this.state
    const { currentCurrency } = this.props
    return (
      <View>
        <View style={styles.inputContainerWrapper}>
          <View style={styles.inputContainer}>
            {!internalPrimaryCurrencyIsCrypto && !!inputValue && <Text style={styles.inputCurrencyText}>{`${getCurrencySymbol(currentCurrency)} `}</Text>}
            <TextInput
            ...
            <View style={styles.action}>
              <TouchableOpacity style={styles.actionSwitch} onPress={this.switchCurrency}>
                ...
                <View styles={styles.switchWrapper}>
                  <Icon name="swap" size={16} color={colors.blue} style={styles.switch} />
                </View>
              </TouchableOpacity>
            </View>
          </View>
    ...
    return (
      <View style={styles.collectibleInputWrapper}>
        <View style={styles.collectibleInputImageWrapper}>
          <CollectibleImage containerStyle={styles.collectibleImage} iconStyle={styles.collectibleImage} collectible={selectedAsset} />
        </View>
      ...
      <SafeAreaView style={styles.wrapper} testID={'amount-screen'}>
        <View style={styles.inputWrapper}>
          <View style={styles.actionsWrapper}>
            ...
            <View style={styles.action}>
              <TouchableOpacity style={styles.actionDropdown} disabled={paymentChannelTransaction || isPaymentRequest} onPress={this.toggleAssetsModal}>
                <Text style={styles.textDropdown}>{selectedAsset.symbol || strings('wallet.collectible')}</Text>
                {!paymentChannelTransaction && (
                  <View styles={styles.arrow}>
                    <Icon name="down" size={16} color={colors.white} style={styles.iconDropdown} />
                  </View>
                )}
              </TouchableOpacity>
            </View>
            <View style={[styles.actionBorder, styles.actionMax]}>
              {!selectedAsset.tokenId && (
                <TouchableOpacity style={styles.actionMaxTouchable} disabled={!paymentChannelTransaction && !estimatedTotalGas} onPress={this.useMax}>
        ...
        <KeyboardAvoidingView style={styles.nextActionWrapper} behavior={'padding'} keyboardVerticalOffset={KEYBOARD_OFFSET} enabled={Device.isIos()}>
...
export default inject(({ store: state, nextProps }) => ({
  ...
  // transactionState: nextProps.transaction || state.transaction,
  transactionState: state.transaction,
  // stateTransaction: state.transaction,
  selectedAsset: state.transaction.selectedAsset,

  setTransactionObject: state.transaction.setTransactionObject,
  prepareTransaction: state.transaction.prepareTransaction,
