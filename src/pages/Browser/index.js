...
import { View, Dimensions, Platform, InteractionManager } from 'react-native';
...
/**
 * React.Component that wraps all the browser
  ...
  takeScreenshot = (url, tabID) =>
    new Promise((resolve, reject) => {
      captureScreen({
        ...
        error => {
          console.error(error, `Error saving tab ${url}`);
          reject(error);
        }
      );
    });
...
export default inject(({ store: state }) => ({
  ...
  closeAllTabs: () => state.browser.closeAllTabs(),
  closeTab: id => state.browser.closeTab(id),
  setActiveTab: id => state.browser.setActiveTab(id),
  updateTab: (id, url) => state.browser.updateTab(id, url),
