...
import { toWei, toBN, renderFromWei, BNToHex } from '../../../utils/number';
// eslint-disable-next-line import/no-nodejs-modules
import { EventEmitter } from 'events';
import AppConstants from './AppConstants';
import byteArrayToHex from '../../../utils/bytes';
...
class PaymentChannelsClient {
  ...
  async setConnext(provider) {
      ...
      externalWallet: {
        ...
        sign: async txMeta => {
          ...
          try {
            ...
            console.log.error(e, 'ExternalWallet::sign');
            throw e;
          }
        }
      },
    ...
    try {
      console.log.log('PC::createClient about to call');
      const connext = await Connext.createClient(opts);
      console.log.log('PC::createClient success');
      this.setState({
        connext,
      ...
      console.log.error(e, 'PC::createClient');
      throw e;
    }
  }
  ...
  async pollConnextState() {
    ...
    // start polling
    try {
      console.log.log('PC::pollConnextState connext.start');
      await connext.start();
      console.log.log('PC::pollConnextState connext.start succesful');
    } catch (e) {
      this.logCurrentState('PC::start');
      console.log.error(e, 'PC::start');
    }
    ...
    connext.on('onStateChange', async state => {
      try {
        ...
        console.log.error(e, 'PC::onStateChange');
      }
    });
  }
  ...
  checkPaymentHistory = async () => {
    ...
    const lastKnownPaymentIDStr = await AsyncStorage.getItem('@QHWallet:lastKnownInstantPaymentID');
    ...
    if (latestPayment) {
      ...
      if (lastKnownPaymentIDStr) {
        ...
        if (lastKnownPaymentID < latestPaymentID) {
          ...
          await AsyncStorage.setItem('@QHWallet:lastKnownInstantPaymentID', latestPaymentID.toString());
        }
      } else {
        // For first time flow
        await AsyncStorage.setItem('@QHWallet:lastKnownInstantPaymentID', latestPaymentID.toString());
  ...
  pollAndSwap = async () => {
    try {
      ...
      console.log.error(e, 'PC::autoswap');
  ...
  async autoSwap() {
    ...
    if (channelState && weiBalance.gt(toBN('0')) && tokenBalance.lte(HUB_EXCHANGE_CEILING)) {
      ...
      console.log.log('PC::pollAndSwap autoSwap exchanging');
      await this.state.connext.exchange(channelState.balanceWeiUser, 'wei');
      console.log.log('PC::pollAndSwap autoSwap exchanging succesful');
      this.setState({ swapPending: false });
    }
  }
  ...
  deposit = async ({ depositAmount }) => {
    ...
    try {
      ...
      console.log.error(e, 'PC::deposit');
      throw e;
    }
  };
  ...
  withdrawAll = async () => {
    try {
      ...
      console.log.error(e, 'PC::withdraw');
    }
  };
  ...
  logCurrentState = prefix => {
    console.log.log(`${prefix}:error - channelState:`, this.state.channelState);
    console.log.log(`${prefix}:error - connextState:`, this.state.connextState);
    console.log.log(`${prefix}:error - runtime:`, this.state.runtime);
    console.log.log(`${prefix}:error - exchangeRate:`, this.state.exchangeRate);
  };
}
...
const instance = {
  ...
  async init(address) {
    ...
    if (SUPPORTED_NETWORKS.indexOf(provider.type) !== -1) {
      initListeners();
      console.log.log('PC::Initialzing payment channels');
      client = new PaymentChannelsClient(address);
      try {
        console.log.log('PC::setConnext', provider);
        await client.setConnext(provider);
        console.log.log('PC::pollConnextState');
        await client.pollConnextState();
        console.log.log('PC::pollAndSwap');
        await client.pollAndSwap();
      } catch (e) {
        client.logCurrentState('PC::init');
        console.log.error(e, 'PC::init');
      }
    }
  },
	/**
