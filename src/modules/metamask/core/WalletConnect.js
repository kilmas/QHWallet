...
const persistSessions = async () => {
  ...
  await AsyncStorage.setItem('@QHWallet:walletconnectSessions', JSON.stringify(sessions));
};
  ...
  constructor(options) {
    ...
    this.walletConnector.on('session_request', async (error, payload) => {
      ...
      try {
        ...
        console.log('requesting session with', sessionData);
...
const instance = {
  async init() {
    const sessionData = await AsyncStorage.getItem('@QHWallet:walletconnectSessions');
    if (sessionData) {
