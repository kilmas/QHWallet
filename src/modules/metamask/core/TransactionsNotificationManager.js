...
import Networks, { isKnownNetwork } from '../../../utils/networks';
import { toChecksumAddress } from 'ethereumjs-util';
import { hexToBN, renderFromWei } from '../../../utils/number';
...
class TransactionsNotificationManager {
  ...
  requestPushNotificationsPermission = async () => {
    const promptCount = await AsyncStorage.getItem('@QHWallet:pushNotificationsPromptCount');
    if (!promptCount || Number(promptCount) < AppConstants.MAX_PUSH_NOTIFICATION_PROMPT_TIMES) {
      ...
      //     AsyncStorage.setItem('@QHWallet:pushNotificationsPromptCount', times.toString());
      //     // In case we want to prompt again after certain time.
      //     AsyncStorage.setItem('@QHWallet:pushNotificationsPromptTime', Date.now().toString());
      //   }
      // });
    }
  };
