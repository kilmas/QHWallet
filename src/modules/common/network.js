...
const ETH_RPC_URL = "https://gateway.qingah.com/eth_node/";
const BTC_RPC_URL = "https://gatewayohio.qingah.com/btc_node/";
const USDT_RPC_URL = "https://gatewayohio.qingah.com/usdt_node/";
...
class DFNetwork {
  ...
  setRPCURLs = _.debounce(async (eth = ETH_RPC_URL, btc = BTC_RPC_URL, usdt = USDT_RPC_URL) => {
    this.ETHRpcUrl = eth;
    this.BTCRpcUrl = btc;
  ...
  fetchRPCURLs = async () => {
    try {
      ...
      await this.setRPCURLs(result.ethUrl, result.btcUrl, result.usdtUrl);
    } catch (error) {}
  };
  get networkType() {
  ...
  baseURL(type = WALLET_API) {
    return BASEURL_MAINNET_QHWALLET + "/" + type;
  }
  get pingURL() {
    return BASEURL_MAINNET_QHWALLET + "/ping";
  }
  ...
  postV2 = async (url, body, type = WALLET_API, headers) => {
    ...
    try {
      ...
      if (error.message && error.message.startsWith("Error Domain=")) {
        ...
        error.message = strings("network-unavailable");
      }
      logger.breadcrumbs(relative, LOGGER_MODULE_NETWORK);
      throw error;
    }
...
function handleResultV2(result, url) {
  ...
  if (_.isPlainObject(result)) {
    ...
    if (
      ...
      (result.hasOwnProperty("success") && result.success === false) || 
...
function defaultError() {
  const error = new Error(strings("common-network-unavailable"));
  return error;
}
...
function generateError(result) {
  ...
  if (result.message && result.message.startsWith("Error Domain=")) {
    ...
    result.message = strings("network-unavailable");
  }
